...
$name = $idmatrics= $class = "";
$name_err = $idmatrics_err = $class_err = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($idmatrics_err) && empty($class_err)){
        // Prepare an update statement
        $sql = "UPDATE student_info SET name=?, idmatrics=?, class=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_idmatrics, $param_class, $param_id);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="idmatrics" class="form-control <?php echo (!empty($idmatrics_err)) ? 'is-invalid' : ''; ?>"><?php echo $idmatrics; ?></textarea>
